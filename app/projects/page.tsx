...
const ProjectsPage = () => {
  ...
  return (
    <div className="min-h-screen p-8">
      ...
      >
        <h1 className="text-5xl font-bold mb-4 text-foreground">My Projects</h1>
        ...
        {filteredProjects.map((project, index) => (
          ...
          >
            <div className="h-48 bg-gradient-to-br from-primary to-accent flex items-center justify-center p-4">
                <h2 className="text-3xl font-bold text-primary-foreground text-center">{project.shortTitle}</h2>
            </div>
            <div className="p-6 flex-grow flex flex-col">
              <h3 className="font-semibold text-xl mb-1 flex-grow text-foreground">{project.title}</h3>
              <p className="text-sm text-primary mb-4">{project.category}</p>
              <div className="flex justify-end gap-4 mt-auto">
                {project.githubUrl && (
                    <Link href={project.githubUrl} target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-foreground transition-colors">
                        <Github />
                    </Link>
                )}
                {project.liveDemoUrl && (
                    <Link href={project.liveDemoUrl} target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-foreground transition-colors">
                        <LinkIcon />
                    </Link>
                )}
              </div>
